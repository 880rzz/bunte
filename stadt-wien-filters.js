...
var T=lang.indexOf('de')===0?{
 ...
 title:'Filter programmes',intro:'Filter by age, topic and Vienna district. For programmes with changing venues, district filtering becomes meaningful only after selecting a specific event.',search:'Search',searchPh:'Programme or keyword',age:'Age',theme:'Topic',district:'District',all:'All',reset:'Reset',results:'results',dynamic:'Variable venue',
...
function intersects(a,b,c){return a<=c&&b>=c}
...
function init(){var grid=document.querySelector('.program-grid');if(!grid)return;var cards=[].slice.call(grid.querySelectorAll(':scope > .card'));if(!cards.length)return;
 ...
 var box=document.createElement('section');box.className='city-filter-panel';box.setAttribute('aria-label',T.title);box.innerHTML='<div class="city-filter-head"><div><h2>'+T.title+'</h2><p>'+T.intro+'</p></div><strong id="cityFilterCount"></strong></div><div class="city-filter-grid"><label><span>'+T.search+'</span><input id="cityFilterSearch" type="search" placeholder="'+T.searchPh+'"></label><label><span>'+T.age+'</span><select id="cityFilterAge">'+AGE.map(function(x){return opt(x[0],x[1])}).join('')+'</select></label><label><span>'+T.theme+'</span><select id="cityFilterTheme">'+opt('all',T.all)+themeOptions()+'</select></label><label><span>'+T.district+'</span><select id="cityFilterDistrict">'+opt('all',T.all)+districtOptions()+'</select></label><button type="button" class="btn ghost city-filter-reset" id="cityFilterReset">'+T.reset+'</button></div>';
 grid.parentNode.insertBefore(box,grid);
 var q=document.getElementById('cityFilterSearch'),a=document.getElementById('cityFilterAge'),th=document.getElementById('cityFilterTheme'),d=document.getElementById('cityFilterDistrict'),count=document.getElementById('cityFilterCount');
 ...
 [q,a,th,d].forEach(function(el){el.addEventListener(el===q?'input':'change',apply)});document.getElementById('cityFilterReset').addEventListener('click',function(){q.value='';a.value='all';th.value='all';d.value='all';apply()});apply();
}
function style(){var s=document.createElement('style');s.textContent='.city-filter-panel{margin:0 0 24px;padding:20px;border:1px solid var(--line,#d2d2d7);background:#fff}.city-filter-head{display:flex;justify-content:space-between;gap:18px;align-items:flex-start;margin-bottom:16px}.city-filter-head h2{margin:0 0 5px;font-size:1.35rem}.city-filter-head p{margin:0;color:var(--muted,#6e6e73);max-width:760px}.city-filter-grid{display:grid;grid-template-columns:2fr repeat(3,1fr) auto;gap:10px;align-items:end}.city-filter-grid label{display:flex;flex-direction:column;gap:5px;font-size:.82rem;font-weight:700}.city-filter-grid input,.city-filter-grid select{width:100%;min-height:44px;padding:9px 11px;border:1px solid var(--line,#d2d2d7);background:#fff;font:inherit}.city-filter-reset{min-height:44px}.program-grid>.card[hidden]{display:none!important}@media(max-width:900px){.city-filter-grid{grid-template-columns:1fr 1fr}.city-filter-reset{width:100%}}@media(max-width:560px){.city-filter-head{display:block}.city-filter-head strong{display:block;margin-top:8px}.city-filter-grid{grid-template-columns:1fr}}';document.head.appendChild(s)}
